...
def evaluate(data, model, name, nbest=None):
    ...
    return speed, acc, p, r, f, pred_results, pred_scores, report
...
def train(data):
    ...
    for idx in range(data.HP_iteration):
        ...
        if total_loss > 1e8 or str(total_loss) == "nan":
            ...
            exit(1)
        # continue
        speed, acc, p, r, f, _,_, report = evaluate(data, model, "dev")
        ...
        # ## decode test
        speed, acc, p, r, f, _,_, report = evaluate(data, model, "test")
...
def load_model_decode(data, name):
    ...
    start_time = time.time()
    speed, acc, p, r, f, pred_results, pred_scores, report = evaluate(data, model, name, data.nbest)
    end_time = time.time()
